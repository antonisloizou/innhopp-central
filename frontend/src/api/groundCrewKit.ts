...
export type GroundCrewKitItem = { key: string; checked: boolean; absent: boolean };
...
export const updateGroundCrewKitItem = (innhoppId: number, key: string, checked: boolean, absent = false) =>
...
export const updateGroundCrewReturnKitItem = (innhoppId: number, key: string, checked: boolean, absent = false) => apiRequest<GroundCrewKit>(`/checklists/innhopps/${innhoppId}/ground-crew-kit/${key}?stage=site_clearing`, { method: 'POST', body: JSON.stringify({ checked, absent }) });
