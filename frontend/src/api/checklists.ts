...
export type ChecklistHistoryEvent = { id:number; item_label:string; role:ChecklistRole; action:'completed'|'reversed'|'overridden'; actor:string; reason?:string; created_at:string };
...
export const reverseChecklistItem = (innhoppId:number,itemId:number,role:ChecklistRole,reason:string) => apiRequest<InnhoppChecklist>(`/checklists/innhopps/${innhoppId}/items/${itemId}/reverse`,{method:'POST',body:JSON.stringify({role,reason})});
