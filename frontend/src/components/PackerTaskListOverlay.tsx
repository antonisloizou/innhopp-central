...
export default function PackerTaskListOverlay({ innhoppId, title, sequence, scheduledAt, taskList: initial, onClose, onUpdated }: Props) {
  ...
  return createPortal(<div className="event-schedule-preview-backdrop ground-kit-backdrop" onClick={onClose} role="presentation"><section className="card overlay-panel-with-close ground-kit-overlay" role="dialog" aria-modal="true" aria-labelledby="packer-task-list-title" onClick={(event) => event.stopPropagation()}>
    ...
    <section className="ground-kit-group ground-task-list"><header><div><h4>Task List</h4></div><span>{checked.size}/{tasks.length}</span></header><div className="ground-kit-items">{tasks.map(([key, label, detail], index) => <label key={key} className={`ground-kit-item${checked.has(key) ? ' checked' : ''}${saving === key ? ' saving' : ''}`}><input type="checkbox" checked={checked.has(key)} disabled={saving !== null} onChange={() => void toggle(key)} /><span className="ground-task-number">{index + 1}</span><span className="ground-kit-item-copy"><strong>{label}</strong>{detail && <small>{detail}</small>}</span><span className="ground-kit-tick">✓</span></label>)}</div></section>
