...
export default function GroundCrewTaskListOverlay({ innhoppId, title, sequence, scheduledAt, taskList: initial, onClose, onUpdated }: Props) {
  ...
  return createPortal(<div className="event-schedule-preview-backdrop ground-kit-backdrop" onClick={onClose} role="presentation"><section className="card overlay-panel-with-close ground-kit-overlay" role="dialog" aria-modal="true" aria-labelledby="ground-task-list-title" onClick={(event) => event.stopPropagation()}>
    ...
    <section className="ground-kit-group ground-task-list"><header><div><h4>Task List</h4></div><span>{checked.size}/{tasks.length}</span></header><div className="ground-kit-items">{tasks.map(([key, label], index) => <Fragment key={key}>{index === 3 && <div className="ground-task-divider"><span>Briefing</span></div>}<div className={`ground-kit-item${checked.has(key) ? ' checked' : ''}${saving === key ? ' saving' : ''}`}><span className="ground-task-number">{index + 1}</span><span className="ground-kit-item-copy"><strong>{label}</strong></span><button type="button" className="ground-kit-tick" aria-label={`Mark ${label} as ${checked.has(key) ? 'unchecked' : 'checked'}`} disabled={saving !== null} onClick={() => void toggle(key)}>✓</button></div></Fragment>)}</div></section>
