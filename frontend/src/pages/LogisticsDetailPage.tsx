...
const LogisticsDetailPage = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteTransport(Number(transportId));
      navigate(-1);
    } catch (err) {
      setMessage(err instanceof Error ? err.message : 'Failed to delete transport');
    }
  };
  ...
  return (
    <section {...editGuardProps}>
        ...
        <div>
          <div className="logistics-detail-title-row">
            <DetailPageLockTitle locked={locked} onToggleLocked={toggleLocked}>
              <h2 className="logistics-detail-title">{routeSummary || 'Transport Route'}</h2>
            </DetailPageLockTitle>
            <span
                        ...
                        {entry.removable ? (
                          <Link
                            to={{
                              ...
                              search: transportId ? `?transportId=${transportId}` : undefined
                            }}
      ...
      {selectedEventId && transportId ? (
        <DetailCostCard
          eventId={Number(selectedEventId)}
          scheduleType="transport"
          scheduleId={Number(transportId)}
          defaultName={`${form.pickup_location || 'Pickup'} -> ${form.destination || 'Destination'}`}
        />
      ) : null}
      {lockNotice}
    </section>
