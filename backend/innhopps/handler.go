...
func scanInnhopp(row pgx.Row) (Innhopp, error) {
	...
	innhopp.FerryFlight = ferryFlight
...
func (h *Handler) getInnhopp(w http.ResponseWriter, r *http.Request) {
	...
	if innhopp.FerryFlight && isParticipantOnlySession(r) {
		httpx.Error(w, http.StatusNotFound, "innhopp not found")
		return
	}
...
func isParticipantOnlySession(r *http.Request) bool {
...
func (h *Handler) updateInnhopp(w http.ResponseWriter, r *http.Request) {
	...
	if p.LandingDistanceByAir != nil {
		if *p.LandingDistanceByAir < 0 {
			httpx.Error(w, http.StatusBadRequest, "landing_distance_by_air must be zero or positive")
			return
		}
		val := *p.LandingDistanceByAir
		landingDistanceByAir = &val
	}
	var landingDistanceByRoad *float64
	if p.LandingDistanceByRoad != nil {
		if *p.LandingDistanceByRoad < 0 {
			httpx.Error(w, http.StatusBadRequest, "landing_distance_by_road must be zero or positive")
			return
		}
		val := *p.LandingDistanceByRoad
		landingDistanceByRoad = &val
	}
	if p.AdditionalLoads < 0 {
		httpx.Error(w, http.StatusBadRequest, "additional_loads must be zero or positive")
		return
	}
	...
	row := h.db.QueryRow(r.Context(),
		`UPDATE event_innhopps
         SET sequence = $1, name = $2, aircraft_id = CASE WHEN $3 THEN $4 ELSE aircraft_id END, coordinates = $5, takeoff_airfield_id = $6, elevation = $7, scheduled_at = $8, notes = $9,
             ...
             landing_airfield_id = $15, landing_distance_by_air = $16, landing_distance_by_road = $17, ferry_flight = $18, additional_loads = $19,
             ...
             risk_assessment = $28, safety_precautions = $29, jumprun = $30, hospital = $31, rescue_boat = $32, minimum_requirements = $33,
             image_files = COALESCE($34::jsonb, image_files), land_owners = $35::jsonb, land_owner_permission = $36
         WHERE id = $37
		...
		reason, adjust, notam, distanceByAir, distanceByRoad, p.LandingAirfieldID, landingDistanceByAir, landingDistanceByRoad, p.FerryFlight, p.AdditionalLoads,
	...
	if innhopp.LandingAirfieldID != nil {
		if _, err := h.db.Exec(
			...
			*innhopp.LandingAirfieldID,
			...
			logUpdateFailure(innhoppID, p, err, "link_landing_airfield")
			httpx.Error(w, http.StatusInternalServerError, "failed to link airfield to event")
			return
		}
	}
	...
	var budgetID int64
	if err := h.db.QueryRow(r.Context(), `SELECT id FROM event_budgets WHERE event_id = $1`, innhopp.EventID).Scan(&budgetID); err != nil && !errors.Is(err, pgx.ErrNoRows) {
		logUpdateFailure(innhoppID, p, err, "find_event_budget")
		httpx.Error(w, http.StatusInternalServerError, "failed to update aircraft budget")
		return
	} else if err == nil {
		if err := budgets.NewHandler(h.db).SyncAutoAircraftLineItems(r.Context(), budgetID); err != nil {
			logUpdateFailure(innhoppID, p, err, "sync_aircraft_budget")
			httpx.Error(w, http.StatusInternalServerError, "failed to update aircraft budget")
			return
		}
	}
